...
However, we did find differences in children's rates of holding for specific object categories across the day. We ran individual linear mixed-effects models, which included fixed effects of site, hour of the day, and their interaction, for each of the six categories. Synthetic objects were marginally more common during the afternoon hours ($\beta$ = `r round(subset(tod.effects, term == "tod" & category == "Synthetic")$estimate, 2)`, _SE_ = `r round(subset(tod.effects, term == "tod" & category == "Synthetic")$std.error, 2)`, _t_ = `r round(subset(tod.effects, term == "tod" & category == "Synthetic")$statistic, 2)`, _p_ = `r round(subset(tod.effects, term == "tod" & category == "Synthetic")$p.value, 3)`)<!--mc2kc: note these effects of synthetic aren't coming out anymore-->, and food items were handled with significantly greater frequency during the morning hours ($\beta$ = `r round(subset(tod.effects, term == "tod" & category == "Food")$estimate, 2)`, _SE_ = `r round(subset(tod.effects, term == "tod" & category == "Food")$std.error, 2)`, _t_ = `r round(subset(tod.effects, term == "tod" & category == "Food")$statistic, 2)`, _p_ = `r round(subset(tod.effects, term == "tod" & category == "Food")$p.value, 3)`; Figure 2). No other main effects or two-way interactions reached statistical significance (all *p*s > 0.05).
