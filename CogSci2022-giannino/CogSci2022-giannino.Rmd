...
## Future directions
<!--mc2kc: this is too long! feel free to pare down as much as you want! even all of it :)-->
One weakness of the current dataset is that we have classified objects on the basis of broadly similar semantics. If we were to reclassify objects based on specific associated activity or applied function, we could better understand how object assemblages and their use by children (a) indicate links between social and linguistic behavior and (b) change over developmental time (e.g., a spoon as a chew toy, musical instrument, and, ultimately, a utensil). Better comparison to existing data also demands we analyze the temporal characteristics of handling bouts and also track unique instances of objects [@herzberg2021exuberant] though we note this will be near-impossible for some object types (e.g., leaves, twigs). We would also like to extend our comparison to other communities, rural and urban, but this will require daylong photo collection in these other sites---there is less overlap between children within the sites studied here compared to @herzberg2021exuberant's US data, but this may be as much due to recording type (e.g., two-hour videos) as to cultural difference. Our most urgent goal is to analyze the speech surrounding these bouts of object handling to derive estimates of how often the objects are talked about, what is mentioned, and by whom. By combining these daylong photo streams with audio data in two unrelated cultural contexts, our aim is to develop a benchmark against which models and mechanisms of word learning via object-centric interaction can be tested.
