...
```{r, data-prep}
raw.data <- read_csv("../data/usable.data_20220115.csv") %>%
...
# Method
## Corpus
We analyze daylong photo streams from child-worn cameras in two rural, small-scale subsistence farming communities: Rossel Island (Papua New Guinea) and Tseltal Maya (Southern Mexico). While these communities are comparable in many ways (e.g., rural, swidden horticulturalist, housed in multi-generation family complexes), there are important differences in the organization of young children's daily lives as well as the availability of certain types of objects (e.g., synthetic materials due to greater market integration in the Tseltal context), that lead us to expect differences in what children handle and when. <!--kc: needs work, will return to this section later; mc2kc: made some edits, change as you like!--> Our daylong photo streams consist of images captured every 15 (Rossel) to 30 (Tseltal) seconds over the course of, typically, 8 (Rossel) to 9 (Tseltal) hours at home. Children wore a recording vest equipped with a camera (Narrative Clip 1) and miniature fisheye lens (Photojojo Super Fisheye) that provided a 180$\text{\textdegree}$ view of the environment. For younger infants who were not yet walking, the camera was instead worn by the primary caregiver. Previously, 83 daylong photo streams (113668 photos) had been comprehensively manually annotated for the presence of child object handling [@casillasURdaylong]; we further annotate and analyze this subset of `r nrow(rawdata)` photos in the present study.
...
```
Children's overall rate of object handling increased significantly with age (Figure 3A). That is, older children handled more unique objects per hour ($\beta$ = `r round(subset(hourly.objects.byage.effects$estimate, hourly.objects.byage.effects$term == "age"), 2)`, _SE_ = `r round(subset(hourly.objects.byage.effects$std.error, hourly.objects.byage.effects$term == "age"), 2)`, _t_ = `r round(subset(hourly.objects.byage.effects$statistic, hourly.objects.byage.effects$term == "age"), 2)`, _p_ = `r round(subset(hourly.objects.byage.effects$p.value, hourly.objects.byage.effects$term == "age"), 3)`). <!--kc: possibly add category distribution across age, which is generally consistent.--> Additionally, with increasing age, children handled more objects from different categories per hour ($\beta$ = `r round(subset(hourly.categories.byage.effects$estimate, hourly.categories.byage.effects$term == "age"), 2)`, _SE_ = `r round(subset(hourly.categories.byage.effects$std.error, hourly.categories.byage.effects$term == "age"), 2)`, _t_ = `r round(subset(hourly.categories.byage.effects$statistic, hourly.categories.byage.effects$term == "age"), 2)`, _p_ = `r round(subset(hourly.categories.byage.effects$p.value, hourly.categories.byage.effects$term == "age"), 3)`). <!--kc: need to check correlations between objects and categories. can also switch to proportions for categories if needed--> These effects were consistent across sites; we found no main effects of site or interactions between site and age (all *p*s > 0.05).
