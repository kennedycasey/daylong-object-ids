...
for (i in c(categories)) {
  ...
  lmer.output <- lmer(n.objects ~ site * eval(as.symbol(tolower(trimws(str_remove(i, "Tool"))))) + (1|sub_num), model.data.incl.zeroes) %>%
  tidy() %>% 
...
ggplot(hourly.objects.bycategory,
  ...
  annotate(geom = "text", label = "*", x = "Immovable", y = 3.5, size = 6) +
  scale_color_manual(values = site.colors) + 
  scale_fill_manual(values = site.colors) +
