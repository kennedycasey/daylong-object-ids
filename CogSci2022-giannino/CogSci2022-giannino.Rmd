...
# Discussion
We annotated and analyzed `r format(total.coded, big.mark = ",", trim = TRUE)` images of at-home child object handling from children under age four in two subsistence communities from opposite sides of the globe. To facilitate comparison, we classified individual objects into broad semantic categories and then examined how long children spent handling objects from each category, as well as how many unique objects per hour they handled across categories. Our main findings are as follows: Overall time spent within categories appeared stable across age and cultural context. In contrast, when it came to unique objects per hour, children handled a greater diversity of synthetic and immovable objects relative to the other categories, and did so to different extents between sites. The rate of transition between objects also varied between sites, with Tseltal children increasing their transition rate over age. Last but not least, time spent with objects is Zipfian, and many of the most common items within site were also common _across_ the two sites. We discuss this rich set of findings with respect to (a) object handling as a viewpoint into children’s worlds in general and (b) the implications of our findings for word learning specifically.
