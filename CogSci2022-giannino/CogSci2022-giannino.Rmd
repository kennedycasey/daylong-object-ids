...
category.props.bytod <- category.data %>%
  group_by(sub_num, object_type, site, tod) %>%
  summarize(n.category = n()) %>%
  distinct() %>%
  ungroup() %>% 
  group_by(sub_num, site, tod) %>%
  summarize(n.images = sum(n.category), 
            n.category = n.category, 
            prop = n.category/n.images, 
            object_type = object_type) %>%
...
for (i in unique(category.props.bytod$object_type)) {
  ...
  #assign(paste0(tolower(i), ".tod.model"), effects)
  #assign(paste0(tolower(i), ".tod.effects"), plot.effects)
}
...
```
When we divide the day into morning, midday, and afternoon, we see largely stable rates of handling across all object categories (Figure 2). We ran individual linear mixed-effects models, which included fixed effects of site, time of day, and their interaction, for all 6 categories. The only significant result was a main effect of time of day for the tool category ($\beta$ = `r round(subset(tod.effects$estimate, tod.effects$term == "todPM" & tod.effects$object_type=="Tool"), 2)`, _SE_ = `r round(subset(tod.effects$std.error, tod.effects$term == "todPM" & tod.effects$object_type=="Tool"), 2)`, _t_ = `r round(subset(tod.effects$statistic, tod.effects$term == "todPM" & tod.effects$object_type=="Tool"), 2)`, _p_ = `r round(subset(tod.effects$p.value, tod.effects$term == "todPM" & tod.effects$object_type=="Tool"), 3)`), where these objects were most frequently handled in the afternoon in both sites. No other main effects or two-way interactions reached statistical significance. Additionally, we examined whether the number of unique objects handled per hour varied as a function of object category and time of day but found no significant main effects or two-way interactions (all *p*s > 0.05).
...
```{r fig2, fig.env = "figure", fig.pos = "h", fig.align = "center", fig.width = 3.4, fig.height = 2.9, fig.cap = "Frequency of handling by object category across different times of day. Point ranges reflect model-predicted mean percentage of handling and 95\\% confidence intervals. Individual points show raw percentages for each child."}

ggplot(tod.effects.toplot, aes(x = tod, y = predicted*100, color = site, fill = site)) +
  facet_wrap(. ~ object_type, nrow = 2) +
  geom_point(category.props.bytod, mapping = aes(x = tod, y = prop*100, color = site, fill = site), size = 0.5, alpha = 0.25, position = position_jitterdodge(dodge.width = 0.9, jitter.width = 0.1)) +
  geom_errorbar(aes(ymin = conf.low*100, ymax = conf.high*100), position = position_dodge(0.9), width = 0.25, size = 0.5, color = "black") +
