...
```
Next, we explored characteristics of object handling across the day. <!--kc: possibly add overall counts of unique objects across hours of the day --> We ran individual linear mixed-effects models, which included fixed effects of site, hour of the day, and their interaction, for all 6 categories. Synthetic objects were marginally more common during the afternoon hours ($\beta$ = `r round(subset(tod.effects$estimate, tod.effects$term == "tod" & tod.effects$object_type == "Synthetic"), 2)`, _SE_ = `r round(subset(tod.effects$std.error, tod.effects$term == "tod" & tod.effects$object_type == "Synthetic"), 2)`, _t_ = `r round(subset(tod.effects$statistic, tod.effects$term == "tod" & tod.effects$object_type == "Synthetic"), 2)`, _p_ = `r round(subset(tod.effects$p.value, tod.effects$term == "tod" & tod.effects$object_type == "Synthetic"), 3)`), and food items were handled with significantly greater frequency during the morning hours ($\beta$ = `r round(subset(tod.effects$estimate, tod.effects$term == "tod" & tod.effects$object_type == "Food"), 2)`, _SE_ = `r round(subset(tod.effects$std.error, tod.effects$term == "tod" & tod.effects$object_type == "Food"), 2)`, _t_ = `r round(subset(tod.effects$statistic, tod.effects$term == "tod" & tod.effects$object_type == "Food"), 2)`, _p_ = `r round(subset(tod.effects$p.value, tod.effects$term == "tod" & tod.effects$object_type == "Food"), 3)`; Figure 2). No other main effects or two-way interactions reached statistical significance (all *p*s > 0.10).

```{r, time-of-day-objects}
hourly.objects.bytod <- object.data %>%
  left_join(object.bycategory, by = "object") %>%
  group_by(site, sub_num, object_type, hour, tod) %>%
  summarize(n.objects = length(unique(object))) %>%
  ungroup() %>%
  mutate(tod = factor(tod, levels = c("morning", "midday", "afternoon"), labels = c("AM", "12", "PM")), 
         object_type = factor(object_type, levels = c("Synthetic", "Food", "Tool", "Toy", "Natural", "Immovable")))

hourly.objects.bytod.effects.list <- list()
hourly.objects.bytod.effects.toplot.list <- list()
for (i in unique(hourly.objects.bytod$object_type)) {
  model <- lmer(n.objects ~ site * tod + (1|sub_num), filter(hourly.objects.bytod, object_type == i))

  effects <- tidy(model) %>%
    mutate(object_type = i) %>%
    filter(effect == "fixed" & term != "(Intercept)" & p.value < 0.05)
  
  effects.toplot <- ggpredict(model, terms = c("site", "tod"), type = "fixed", back.transform = TRUE) %>%
  rename(tod = group, site = x) %>%
  mutate(object_type = i)
  
  hourly.objects.bytod.effects.list[[i]] <- effects
  hourly.objects.bytod.effects.toplot.list[[i]] <- effects.toplot
}

hourly.objects.bytod.effects <- do.call(rbind, hourly.objects.bytod.effects.list)
hourly.objects.bytod.effects.toplot <- do.call(rbind, hourly.objects.bytod.effects.toplot.list) %>%
  mutate(object_type = factor(object_type, levels = c("Synthetic", "Food", "Tool", "Toy", "Natural", "Immovable")))
```
