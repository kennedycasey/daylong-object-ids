...
Early access to objects is shaped by culture-specific practices for
...
Ochs, \& Ragazzini (2012){]}. We might infer, based on these assemblages
of home objects, that much of what children do and talk about at home is
...
synthetic object category (\(\beta\) = 0.38, \emph{SE} = 0.09, \emph{t}
= 4.19, \emph{p} \textless{} 0.001) and a significant site-by-synthetic
interaction (\(\beta\) = 0.37, \emph{SE} = 0.12, \emph{t} = 2.97,
\emph{p} = 0.057) such that children handled more unique synthetic
...
0.14, \emph{t} = -3.29, \emph{p} = 0.021) and work tool (\(\beta\) =
-0.71, \emph{SE} = 0.19, \emph{t} = -3.81, \emph{p} = 0.003) categories,
...
object category (\(\beta\) = 0.49, \emph{SE} = 0.12, \emph{t} = 4.23,
\emph{p} \textless{} 0.001) and site-by-immovable interaction (\(\beta\)
= -0.88, \emph{SE} = 0.18, \emph{t} = -4.98, \emph{p} \textless{} 0.001)
...
Rossel children than for Tseltal children (Figure 4B).
...
variation in total available images for each child (range = 1--626) and
...
(\(\beta\) = 0.11, \emph{SE} = 0.03, \emph{t} = 3.18, \emph{p} = 0.002;
...
increase (\(\beta\) = 0.01, \emph{SE} = 0, \emph{t} = 1.43, \emph{p} =
0.157). However, there was a significant main effect of site (\(\beta\)
= -0.44, \emph{SE} = 0.14, \emph{t} = -3.26, \emph{p} = 0.002) as well
as a site-by-age interaction (\(\beta\) = 0.01, \emph{SE} = 0, \emph{t}
= 2.56, \emph{p} = 0.013), indicating that Tseltal children made fewer
transitions between objects per hour than Rossel children but showed a
steeper increase across age (Figure 6B). At the category level, we found
that, with increasing age, children made significantly more transitions
between object categories per hour (\(\beta\) = 0.01, \emph{SE} = 0.01,
\emph{t} = 1.77, \emph{p} = 0.08), with no detectable differences across
sites.
