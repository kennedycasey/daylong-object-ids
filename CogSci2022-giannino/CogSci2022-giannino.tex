...
\& Elliott, 2021); we further annotate and analyze the subset of 15364
...
ranging from 1 to 622 (\emph{M}\textsubscript{\emph{Rossel}} = 222.5,
\emph{M}\textsubscript{\emph{Tseltal}} = 190.5).
...
for annotators to identify handled objects (124 images, 0.81\% of the
data set), if annotators were otherwise unsure about what objects were
being handled (231, or 1.5\%), if there was no handled object (26, or
...
identifiable. In total, 15364 images were deemed usable by annotators
(8676 for Rossel, 6669 for Tseltal).
...
Object Category & Rossel & Tseltal \\ 
  \hline
Synthetic & shirt, woven basket, container & shirt, pants, blanket \\ 
...
Children handled an average of 26.99 unique objects per day (median =
26, \emph{SD} = 15.87, range = 1--62), with no significant differences
across sites (\emph{M}\textsubscript{\emph{Rossel}} = 26.33,
\emph{M}\textsubscript{\emph{Tseltal}} = 27.71, \emph{W} = 662, \emph{p}
= 0.828). The distribution of handled objects was highly right-skewed
within and across children. Each child's distribution was skewed such
that a small group of objects were handled in a majority of their images
but most were handled for only short periods of time (Figure 2). Across
children, common objects followed a similar Zipfian distribution: some
objects were handled by many children, but most objects were only
handled by 1-2 children in each site (Rossel: 61.51\%, Tseltal:
66.33\%).
...
Comparing across sites, 29.55\% of objects were present in both
...
(e.g., rope, shirt, container, etc.,
\emph{M}\textsubscript{\emph{Rossel}} = 32.4\%,
\emph{M}\textsubscript{\emph{Tseltal}} = 36.78\% of handling) and food
(\emph{M}\textsubscript{\emph{Rossel}} = 27.75\%,
\emph{M}\textsubscript{\emph{Tseltal}} = 32.75\% of handling). For 57 of
...
(\emph{M}\textsubscript{\emph{Rossel}} = 10.18\%,
\emph{M}\textsubscript{\emph{Tseltal}} = 3.27\%, adjusted \emph{p} =
...
During any given hour, children handled 6.5 objects from 3.16 different
...
of the synthetic object category (\(\beta\) = 1.09, \emph{SE} = 0.11,
\emph{t} = 10.13, \emph{p} \textless{} 0.001) along with a marginal
...
-0.44, \emph{SE} = 0.13, \emph{t} = -3.51, \emph{p} = 0.007), tool
(\(\beta\) = -0.47, \emph{SE} = 0.14, \emph{t} = -3.44, \emph{p} =
0.008) , and toy (\(\beta\) = -0.82, \emph{SE} = 0.18, \emph{t} = -4.56,
...
-0.86, \emph{SE} = 0.23, \emph{t} = -3.73, \emph{p} = 0.003; Figure 4B).
...
hour (\(\beta\) = 0.11, \emph{SE} = 0.03, \emph{t} = 3.1, \emph{p} =
...
\emph{SE} = 0.01, \emph{t} = 4.34, \emph{p} = 0). These effects were
...
\emph{t} = 1.59, \emph{p} = 0.117). However, this analysis revealed a
significant main effect of site (\(\beta\) = -0.41, \emph{SE} = 0.14,
\emph{t} = -3.07, \emph{p} = 0.003) as well as a site-by-age interaction
(\(\beta\) = 0.01, \emph{SE} = 0, \emph{t} = 2.31, \emph{p} = 0.024)
...
per hour (\(\beta\) = 0.02, \emph{SE} = 0.01, \emph{t} = 2.28, \emph{p}
= 0.025), with no detectable differences across sites (Figure 5D).
