...
# read in data
annotations <- read_csv("data/casillas/reliability-annotations.csv")
...
durations <- read_csv("data/casillas/metadata/timestamps.csv") %>%
...
if (nrow(not.checked.unsure > 0)) {
  write_csv(not.checked.unsure, "data/casillas/manual-checks/new-reliability-unsure-objects.csv")
}
...
if (nrow(not.checked.none > 0)) {
  write_csv(not.checked.none, "data/casillas/manual-checks/new-reliability-no-objects.csv")
}
...
if (nrow(not.regularized > 0)) {
  write_csv(not.regularized, "data/casillas/manual-checks/new-reliability-objects-to-label.csv")
}
...
if (nrow(not.categorized > 0)) {
  write_csv(not.categorized, "data/casillas/manual-checks/new-reliability-objects-to-categorize.csv")
}
...
write_csv(data.to.export, "data/casillas/reliability-data.csv")
}

# CALCULATE RELIABILITY STATS ---------------------------------------------
reliability <- read_csv("data/casillas/reliability-data.csv") %>%
  group_by(sub_num, image) %>%
...
main <- read_csv("data/casillas/all-data.csv") %>%
...
compare <- main %>%
  ...
  write_csv("data/casillas/reliability-stats.csv")